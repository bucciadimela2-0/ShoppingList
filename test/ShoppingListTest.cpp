...
protected:
    ...
    void SetUp() override {
        ...
        shoppingList = std::make_shared<ShoppingList>(std::string("lista1"));
...
TEST_F(ShoppingListTest, AttachAndDetach) {
    ...
    std::shared_ptr<User> observer1 = std::make_shared<User>(std::string("greg"));
    std::shared_ptr<User> observer2 = std::make_shared<User>(std::string("dalila"));
